...
async function generateUserReport() {
    ...
    buyers.forEach((buyer, i) => {
        ...
        buyerSheet.addRow({
            ...
            email: buyer.email,
            phone: buyer.phone || '—',
            address: buyer.location?.address ? `${buyer.location.address}, ${buyer.location.city || ''}, ${buyer.location.state || ''} - ${buyer.location.pincode || ''}`.replace(/,\s*,/g, ',') : '—',
    ...
    sellerSheet.columns = [
        ...
        { header: 'Shop PIN Code', key: 'shopPincode', width: 16 },
    ...
    sellers.forEach((seller, i) => {
        ...
        sellerSheet.addRow({
            ...
            shopAddress: shop?.location?.address ? `${shop.location.address}, ${shop.location.city || ''}, ${shop.location.state || ''} - ${shop.location.pincode || ''}`.replace(/,\s*,/g, ',') : '—',
            shopPincode: shop?.location?.pincode || seller.location?.pincode || '—',
